...
def load_bert_features_claim_inference(input, size=5):
    features, claims = [], []

    if len(input['evidences']) > size:
        input['evidences'] = input['evidences'][:size]
    
    feature = feature_pooling(input['evidences'], size)
    features.append(feature)
    claims.append(input['claim'])

    features_torch = torch.FloatTensor(features)
    claims_torch = torch.FloatTensor(claims)

    return features_torch, claims_torch
